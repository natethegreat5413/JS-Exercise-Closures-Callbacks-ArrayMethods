...
function isItAnApple(strings) {
  const newArray = strings.map(function(fruit){
    if(fruit === 'apple'){
      return true;
    }else{
      return false;
...
function removeApple(strings) {
  const lastApple = strings.filter(function(fruit){
    if(fruit !== 'apple'){
      return fruit;
    }
  })
...
function stringSmash(strings) {
  return strings.reduce((accumulator,index) => accumulator + index);
}
...
function getFullNames(runners) {
  const fullNames = runners.map(function(runners){
